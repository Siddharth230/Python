...
def find_cheapest_fruit(fruit_prices: dict) -> str:
    ...
    """
    fruit_prices_tuples = list(fruit_prices.items())
    min_fruit, min_price = fruit_prices_tuples[0]
    for fruit, price in fruit_prices_tuples[1:]:
        if price < min_price:
...
def group_fruits(fruits: list):
    ...
    """
    fruit_with_first_letter = {}
    for fruit in fruits:
        if fruit[0] not in fruit_with_first_letter:
            fruit_with_first_letter[fruit[0]] = []
        fruit_with_first_letter[fruit[0]].append(fruit)
    return {
        letter: sorted(fruits) for letter, fruits in fruit_with_first_letter.items()
    }
